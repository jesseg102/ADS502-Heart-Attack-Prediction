...
data$slp_cat<-c("0"="upsloping","1"="flat", "2"="downsloping")[data$slp]
...
plot(data$sex, main = "Distribution of Sex", ylab = 'Frequency', xlab = 'Sex', names = c("Female","Male"), col=plot_color)
```
...
boxplot(data$trtbps, main = "Distribution of Resting Blood Pressure", ylab = "Resting Blood Pressure (mmHg)", col=plot_color)
```
```{r}
...
```{r}
plot(data$slp, main = "Distribution of Exercise Test Results", xlab = 'Angiographic Results', ylab = 'Frequency', 
     names = c("Upsloping","Flat","Downsloping"), col=plot_color)

```
```{r}
plot(factor(data$thall_cat), main = "Distribution of Thallasemia", xlab = 'Type', ylab = 'Frequency', 
     names = c("Null","Fixed Defect", "Normal", "Reversable Defect"), col=plot_color)
...
```{r}
#the less chance group is slightly underrepresented
...
# Summary of data exploration: there are no true outliers in the data. Can leave the data as is.
...
fx_table7 <- table(data$slp)
prop.table(fx_table7)

fx_table8 <- table(factor(data$thall_cat))
prop.table(fx_table8)
...
```{r}
#both classes are well represented, no need to balance, majority class is high risk 54.5% 
...
```{r}
data_test$cartpred <- factor(predict_data, ordered = TRUE)
...
cartROC <- roc(data_test$output ~ data_test$cartpred,plot=TRUE,print.auc=TRUE,
